...
function addClient(clientData) {
    // Vérifier la limite
    if (!canAddClient()) {
        showAlert(`Limite atteinte (${currentUser.clientLimit} clients maximum)`, 'error');
        return;
    }
...
function updateClient(clientId, clientData) {
    ...
    const emailExists = otherClients.some(client => client.email.toLowerCase() === clientData.email.toLowerCase());
    const phoneExists = clientData.phone && otherClients.some(client => client.phone === clientData.phone);
...
function canAddClient() {
    return currentUser.type === 'premium' || clients.length < currentUser.clientLimit;
}
        ...
        tableHTML += `
            <tr>
                ...
                <td>
                    ...
                    ${client.phone ? `<div><i class="fas fa-phone"></i> ${escapeHtml(client.phone)}</div>` : ''}
                </td>
                <td><span class="status-badge status-${client.status}">${getStatusText(client.status)}</span></td>
                <td><small>${formatDate(client.createdAt)}</small></td>
                <td>
...
function exportToCSV() {
    ...
    const headers = ['ID Client', 'Nom', 'Email', 'Téléphone', 'Statut', 'Projets totaux', 'Projets en cours', 'Projets terminés', 'Date création'];
    let csvContent = BOM + headers.join(';') + '\n';
    
    clients.forEach(client => {
        const clientProjects = projects.filter(project => project.clientId === client.id);
        const totalProjects = clientProjects.length;
        const activeProjects = clientProjects.filter(p => p.status === 'en_cours').length;
        const completedProjects = clientProjects.filter(p => p.status === 'terminé').length;
    ...
    link.download = `clients_detailed_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
